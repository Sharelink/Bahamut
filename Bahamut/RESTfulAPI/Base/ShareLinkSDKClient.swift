...
public protocol ClientProtocal
{
    func setClientStart()
    func setClientClose()
    func execute<T:EVObject>(request:ShareLinkSDKRequestBase,callback:(result:SLResult<T>)->Void) -> Bool
    func execute<T:EVObject>(request:ShareLinkSDKRequestBase,callback:(result:SLResult<[T]>)->Void) -> Bool
    
    func execute(request:ShareLinkSDKRequestBase,callback:(result:Result<String,NSError>)->Void) -> Bool
}
...
public class ShareLinkSDKClient : ClientProtocal
{
    ...
    func setReqHeader(req:ShareLinkSDKRequestBase) -> ShareLinkSDKRequestBase
    {
        req.headers.updateValue(userId, forKey: "userId")
        req.headers.updateValue(token, forKey: "token")
        req.version = SharelinkSDK.version
        return req
    }
    
    private func setReqApi(req:ShareLinkSDKRequestBase)
    {
        req.api = req.apiServerUrl != nil ? req.apiServerUrl + req.api : self.apiServer + req.api
    }
    
    public func execute(request: ShareLinkSDKRequestBase, callback: (result: Result<String,NSError>) -> Void) -> Bool
    {
    ...
    public func execute<T:EVObject>(request:ShareLinkSDKRequestBase,callback:(result:SLResult<[T]>)->Void) -> Bool
    {
        if clientStarted == false || request.getCurrentRequestCount() >= request.getMaxRequestCount()
        {
            let err = SLResult<[T]>()
